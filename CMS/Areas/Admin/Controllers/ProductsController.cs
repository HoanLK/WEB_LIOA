...
namespace CMS.Areas.Admin.Controllers
{
    ...
    public class ProductsController : Controller
    {
        ...
        [AllowAnonymous]
        public JsonResult GetByOtherCategory(int id)
        {
            List<Product> model = new List<Product>();

            //id ở đây là id của Category hiện tại
            var categories = db.CategoryProduct.ToList();
        ...
        [AllowAnonymous]
        public JsonResult Get4NewByCategory(int id)
        {
            var model = db.Product.Where(p => p.idCategoryProduct == id && p.published==1).OrderByDescending(p => p.timeModified).Take(4);
