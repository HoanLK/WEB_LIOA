...
namespace CMS.Areas.Admin.Controllers
{
    public class ProductsAPIController : ApiController
    {
        ...
        public IQueryable<Product> GetProduct(string att, string value)
        {
            ...
            {
                int idCategoryProduct = int.Parse(value);
                var model = db.Product.Where(p => p.published == 1).OrderByDescending(p => p.timeModified).Take(12);
            ...
            {
                int idCategoryProduct = int.Parse(value);
                var model = db.Product.Where(p => p.idCategoryProduct == idCategoryProduct && p.published == 1 && p.feature.Contains("1")).OrderByDescending(p => p.timeModified).Take(4);

                return model;
            }

            if (att == "feature" && att != null && value != null)
            {
                string feature = value;
                var model = db.Product.Where(p => p.feature == feature && p.published == 1);
